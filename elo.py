...
ri, bi = team_ind['Los Angeles Rams'], team_ind['Buffalo Bills']
w, last_w = '2022-1', w
elo['Reset Week ' + w[:4]] = reset(reset_factor, elo[last_w])
last_w = 'Reset Week ' + w[:4] # last week set to reset week
K = 32
ones = np.array([1]*32)
elo_diffs = np.outer(elo[last_w],ones) - np.outer(elo[last_w],ones).T
expected_score_matrix = 1/(1 + 10**(-1*elo_diffs/200))
expected_score_matrix[ri,bi]
input('press enter')
...
def elo_MSE(elo, results):
    ...
    for w_ind in range(1, len(weeks)):
        ...
        scores_half = np.triu(actual_score_matrix, 1)**2 # we don't need to count games twice
        SSE += np.sum(scores_half)

    return SSE/(len(weeks)-1)


K_range = np.linspace(10,40,31)
logistic_range = np.linspace(50, 800, 16)
reset_range = np.linspace(0, 1, 21)

from tqdm.auto import tqdm
results = np.zeros((31, 16, 21))
for k in tqdm(range(len(K_range))):
    for l in tqdm(range(len(logistic_range))):
        for r in tqdm(range(len(reset_range))):
